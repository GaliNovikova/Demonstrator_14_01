...
function openSummary(){
	...
	$('form').on( "submit", function( event ){
		...
		showSearchResultByQuery($(this).find('input').text());
	});
	
	function showSearchResultByQuery(str){
			var searchstr = str.split(' ');
...
/*--------------------------End On document ready--------------------------------*/
		...
		var fieldErrors={
			'iPhone':'iPhone|iPhone|ifoon|ifone|jfone|ijphone|ifo',
			'Samsung S3':'Samsung S3|i8190|Siii|S111'
		};
		readFile('typeahead-list', 'js/list.json',fieldErrors);
		/**moved to brand selected*/
		readFileUser('list-problem', ["Audio (geluid) tijdens het bellen","Audio (geluid) overigen", "Vindt geen Wifi netwerk", "Geen netwerk","Accu snel leeg", "Valt uit", "Behuizing", "Laadt niet op"]);
		...
		$('.close-button.first').on('click', function(){
			...
			//showSearchResultByQuery('');
			$('.brand-search input.form-control.typeahead-list').trigger('keyup');
		...
		$('.close-button.second').on('click', function(){
			...
			$('.brandSelection').show();/*block for search*/
			document.body.scrollTop = 0;
			$('.brand-search input.form-control.typeahead-list').val( curBrand );/*needs to be checked*/
			showSearchResultByQuery(curBrand);
			deleteHashVal('pr');
			deleteHashVal('md');
		});
		...
		$('.close-button.third').on('click', function(){
			...
			$('.brandSelection').show();/*block for search*/
			document.body.scrollTop = 0;
			$('.brand-search input.form-control.typeahead-list').val( curModel );
			showSearchResultByQuery(curModel);
			deleteHashVal('pr');
		});
...
var curProblem = '';
	...
	function showModelByBrand(container){
		...
		$('.brand-search input.form-control.typeahead-list').val(currentBrandName);
		showSearchResultByQuery(currentBrandName);
	...
	$('.companies .problem .img-container').each(function(index){
		...
		function setCurrentProblem(self){
				...
				$('.brand-search input.form-control.typeahead-list').val(curModel +' '+ curProblem);
				showSearchResultByQuery(curModel +' '+ curProblem);
...
/*-------------------------------Typeahed start-----------------------------------*/		
		...
		$('input.form-control.typeahead-list').on('keyup', function(){
			...
			if(val) {
				...
				showSearchResultByQuery(self.value.toLowerCase());
			}
			else{
				/*document.body.scrollTop=0;
